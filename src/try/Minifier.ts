...
export class Minifier {
    constructor(srcDir: string, destDir: string, interfaceFileArr: string[]) {
        this._srcDir = path.normalize(srcDir)
    ...
    compileProject(obfuscation = false, compilerOptions = defaultCompilerOptions) {
        ...
        this.generateDestFiles(renameArr, contentArr)
    }
    ...
    private renameAllReferences(refNodeArr: RefNode[], renameSetArr: Set<string>[], sigSet: Set<string>) {
        ...
        for (const refNode of refNodeArr) {
            ...
            const length = refNode.name.length
    ...
    private generateDestFiles(renameArr: RenameNode[][], contentArr: string[]) {
        const encoder = new TextEncoder()
        const { _srcDir, _destDir, _fileArr, _decFileArr } = this
        ...
        for (let i = 0; i < renameArr.length; i++) {
            ...
            for (const renameNode of fileRenameArr) {
                const buffer = encoder.encode(content.substring(from, renameNode.pos))
                modified.push(buffer, renameNode.name)
                from = renameNode.end
            }
